...
import type { StatutProfessionnel } from "@/types/database.types"
import Link from "next/link"

const PAGE_SIZE = 6

export default function StatutsProfessionnelsPage() {
  const [items, setItems] = useState<StatutProfessionnel[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [selectedItem, setSelectedItem] = useState<StatutProfessionnel | null>(null)
  ...
  const fetchItems = async () => {
    setIsLoading(true)
    const { data, error } = await supabase.from('statuts_professionnels').select('*').order('ordre')
    if (error) {
  ...
  const handleCreate = async () => {
    ...
    setFeedback(null)
    const { error } = await supabase.from('statuts_professionnels').insert(formData)
    if (error) {
      setFeedback({ type: "error", message: `Création échouée: ${error.message}` })
    } else {
      setFeedback({ type: "success", message: "Statut créé avec succès." })
  ...
  const handleUpdate = async () => {
    ...
    setIsSubmitting(true)
    setFeedback(null)
    const { error } = await supabase.from('statuts_professionnels').update(formData).eq('id', selectedItem.id)
    if (error) {
      setFeedback({ type: "error", message: `Modification échouée (${selectedItem.libelle}): ${error.message}` })
    } else {
      setFeedback({ type: "success", message: `Statut "${selectedItem.libelle}" mis à jour.` })
      fetchItems(); setDialogAction(null); resetForm()
    }
    setIsSubmitting(false)
  }
  ...
  const handleDelete = async () => {
    ...
    setIsSubmitting(true)
    setFeedback(null)
    const { error } = await supabase.from('statuts_professionnels').delete().eq('id', selectedItem.id)
    if (error) {
      setFeedback({ type: "error", message: `Suppression échouée (${selectedItem.libelle}): ${error.message}` })
    } else {
      setFeedback({ type: "success", message: `Statut "${selectedItem.libelle}" supprimé.` })
  ...
  const openEditDialog = (item: StatutProfessionnel) => {
  ...
  return (
    <AdminWrapper>
      <div className="p-6">
        <div className="mb-6">
          ...
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Statuts professionnels</h1>
              <p className="text-gray-500">Gérez les statuts professionnels des alumni</p>
            </div>
      ...
      <Card>
        <CardHeader><CardTitle>Liste des statuts ({items.length})</CardTitle></CardHeader>
        <CardContent>
          {feedback && (
          ...
          {isLoading ? (
            ...
            <Table>
              ...
              <TableBody>
                {items.length === 0 ? (
                  <TableRow><TableCell colSpan={3} className="text-center py-8 text-gray-500">Aucun statut</TableCell></TableRow>
                ) : (
      ...
      <Dialog open={dialogAction === 'create' || dialogAction === 'edit'} onOpenChange={() => { setDialogAction(null); resetForm() }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{dialogAction === 'create' ? 'Nouveau statut' : 'Modifier le statut'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label>Libellé *</Label>
              <Input value={formData.libelle} onChange={(e) => setFormData({ ...formData, libelle: e.target.value })} placeholder="Ex: Salarié" />
            </div>
            <div className="space-y-2">
