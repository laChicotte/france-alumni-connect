...
export default function TypesFormationsPage() {
  const [items, setItems] = useState<TypeFormation[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [selectedItem, setSelectedItem] = useState<TypeFormation | null>(null)
  ...
  const fetchItems = async () => {
    setIsLoading(true)
    const { data, error } = await supabase.from("types_formations").select("*").order("ordre")
    if (error) setFeedback({ type: "error", message: `Chargement impossible: ${error.message}` })
    else setItems((data || []) as TypeFormation[])
    setIsLoading(false)
  }

  const handleCreate = async () => {
    setIsSubmitting(true); setFeedback(null)
    const { error } = await supabase.from("types_formations").insert(formData)
    if (error) setFeedback({ type: "error", message: `Création échouée: ${error.message}` })
    else { setFeedback({ type: "success", message: "Type de formation créé." }); fetchItems(); setDialogAction(null); resetForm() }
  ...
  const handleUpdate = async () => {
    ...
    setIsSubmitting(true); setFeedback(null)
    const { error } = await supabase.from("types_formations").update(formData).eq("id", selectedItem.id)
    if (error) setFeedback({ type: "error", message: `Modification échouée: ${error.message}` })
    else { setFeedback({ type: "success", message: `Type "${selectedItem.libelle}" mis à jour.` }); fetchItems(); setDialogAction(null); resetForm() }
    setIsSubmitting(false)
  }
  ...
  const handleDelete = async () => {
    if (!selectedItem) return
    setIsSubmitting(true); setFeedback(null)
    const { error } = await supabase.from("types_formations").delete().eq("id", selectedItem.id)
    if (error) setFeedback({ type: "error", message: `Suppression échouée: ${error.message}` })
    else { setFeedback({ type: "success", message: `Type "${selectedItem.libelle}" supprimé.` }); fetchItems(); setDialogAction(null); setSelectedItem(null) }
    setIsSubmitting(false)
  }

  const resetForm = () => { setFormData({ libelle: "", ordre: 0 }); setSelectedItem(null) }

  const openEditDialog = (item: TypeFormation) => {
    setSelectedItem(item); setFormData({ libelle: item.libelle, ordre: item.ordre }); setDialogAction("edit")
  }
      ...
      <div className="p-6">
        <div className="mb-6">
          ...
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Types de formations</h1>
              <p className="text-gray-500">Gérez les types de formations (Atelier, Masterclass, Webinaire...)</p>
            </div>
                  ...
                  {items.length === 0 ? (
                    ...
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="text-right">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => { setSelectedItem(item); setDialogAction("delete") }}><Trash2 className="h-4 w-4 text-red-500" /></Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label>Libellé *</Label>
                <Input value={formData.libelle} onChange={(e) => setFormData({ ...formData, libelle: e.target.value })} placeholder="Ex: Atelier, Masterclass..." />
              </div>
              <div className="space-y-2">
